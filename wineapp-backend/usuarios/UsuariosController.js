...
router.post('/usuarios', (req, res) => {
    const { nome_vinicola, cnpj, email, telefone_vinicola, password, cep } = req.body;
    ...
    Usuario.create({
        ...
        telefone_vinicola,
        password,
        cep, // Inclua o campo cep no objeto
    })
...
// Rota para buscar o usuário pelo CEP
router.get('/usuarios/cep/:cep', (req, res) => {
    ...
    Usuario.findOne({
        ...
        .then((usuario) => {
            if (usuario) {
                const { id, email, nome_vinicola } = usuario; // Adiciona as propriedades id e email
                res.status(200).send({ id, email, nome_vinicola }); // Retorna id, email e nome_vinicola
            } else {
                res.status(404).send('Usuário não encontrado para o CEP fornecido');
            }
        })
        .catch((error) => {
            console.error(error);
